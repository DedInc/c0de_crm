...
from core.config import CRM_API_URL, DEFAULT_LANGUAGE, INTERNAL_API_KEY
...
async def _save_user_language(user_id: int, language: str) -> bool:
    ...
    try:
        ...
        async with (
            aiohttp.ClientSession() as session,
            session.post(
                url, json={"telegramId": str(user_id), "language": language}, headers=headers
            ) as response,
        ):
            if response.status == 200:
                return True
...
def get_text(key: str, user_id: int, **kwargs: object) -> str:
    ...
    if kwargs:
        try:
            # F4-1: HTML-escape all string kwargs to prevent injection via parse_mode="HTML"
            safe_kwargs = {
                k: html_escape(str(v)) if isinstance(v, str) else v for k, v in kwargs.items()
            }
...
def get_text_by_lang(key: str, lang: str, **kwargs: object) -> str:
