...
async function sendTelegramVerification(telegramId: string): Promise<{ success: boolean; error?: string; errorCode?: string }> {
	try {
		...
		const response = await fetch(`${getBotWebhookUrl()}/verify-telegram`, {
			method: 'POST',
			headers,
...
export const telegramProcedures = {
	updateOwnTelegramId: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			// F2-3: Prevent claiming another user's Telegram ID
			if (input.telegramId !== null) {
				...
				if (existing[0] && existing[0].id !== ctx.user!.id) {
					throw new TRPCError({ code: 'CONFLICT', message: 'This Telegram ID is already linked to another account' });
				}
			}
