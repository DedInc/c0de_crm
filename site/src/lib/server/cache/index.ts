...
const cacheConfig = {
	...
	redis: {
		...
		retryDelay: parseInt(env.REDIS_RETRY_DELAY || '100', 10),
...
// Tracks inflight fetcher calls to prevent cache stampede
const inflightRequests = new Map<string, Promise<unknown>>();

// Safety timeout for inflight deduplication (prevents eternal promises if fetcher hangs)
...
export async function cacheGetOrSet<T>(
	...
	// Create the fetch promise with a safety timeout and register it
	const fetchPromise = Promise.race([
		fetcher(),
		new Promise<never>((_, reject) =>
			setTimeout(() => reject(new Error(`Cache fetcher timeout for key: ${key}`)), INFLIGHT_TIMEOUT_MS)
		)
	])
		.then((fresh) => {
