...
def _require_env(name: str, description: str) -> str:
    ...
    if not value:
        print(f"❌ Missing required env var: {name} — {description}", file=sys.stderr)
        print("   Set it in .env or as a system environment variable.", file=sys.stderr)
        sys.exit(1)
    return value


BOT_TOKEN = _require_env("BOT_TOKEN", "Telegram bot token from @BotFather")
DATABASE_URL = _require_env("DATABASE_URL", "PostgreSQL connection string")
INTERNAL_API_KEY = _require_env("INTERNAL_API_KEY", "Shared secret for CRM <-> Bot communication")
