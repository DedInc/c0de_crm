...
async def process_marker_selection(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback_data is None or callback_message is None:
        ...
        return
    marker_id = callback_data.split(":")[1]
...
async def process_markers_pagination(callback: CallbackQuery, state: FSMContext) -> None:
...
async def process_markers_page_info(callback: CallbackQuery) -> None:
    """Handle click on page info button (do nothing)."""
    await callback.answer()


@router.callback_query(F.data == "markers_done", OrderCreation.markers)
async def process_markers_done(callback: CallbackQuery, state: FSMContext) -> None:
...
async def process_payment(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback_data is None or callback_message is None:
        await callback.answer()
        return
    payment_method = callback_data.split(":")[1]
...
async def process_confirm(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        await callback_message.edit_text(get_text("order_created", user_id))
        set_last_message_id(user_id, callback_message.message_id)
...
async def process_cancel(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.clear()
    clear_active_chat(user_id)
    await callback_message.edit_text(get_text("order_cancelled", user_id))
    set_last_message_id(user_id, callback_message.message_id)
    await callback.answer()
