...
export const revokeOrderPermission = permissionProcedure('assign_orders')
	.input((v) => {
		...
		if (!check.Check(v))
			throw new TRPCError({ code: 'BAD_REQUEST', message: 'Invalid input' });
		return v as { permissionId: string; orderId: string };
	})
	.mutation(async ({ input }) => {
		...
		if (permissionResult[0].orderId !== input.orderId) {
			throw new TRPCError({ code: 'FORBIDDEN', message: 'Permission does not belong to this order' });
		}

		await db.delete(schema.orderPermissions)
			.where(
				and(
					eq(schema.orderPermissions.id, input.permissionId),
					eq(schema.orderPermissions.orderId, input.orderId)
				)
			);
