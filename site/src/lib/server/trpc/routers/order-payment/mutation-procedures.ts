...
export const mutationProcedures = {
	sendPaymentInfo: permissionProcedure('send_payment_info')
		...
		.mutation(async ({ input, ctx }) => {
			...
			await db.transaction(async (tx) => {
				await tx.insert(schema.orderPaymentInfo).values({
					id,
	...
	updatePaymentInfo: permissionProcedure('send_payment_info')
		...
		.mutation(async ({ input, ctx }) => {
			...
			// F2-5: Only the original provider or admin can modify payment records
			if (existing[0].providedByUserId !== ctx.user!.id && !isAdmin(ctx.user)) {
				throw new TRPCError({ code: 'FORBIDDEN', message: 'Can only modify your own payment records' });
			}
	...
	deletePaymentInfo: permissionProcedure('send_payment_info')
		...
		.mutation(async ({ input, ctx }) => {
			...
			// F2-5: Only the original provider or admin can delete payment records
			if (existing[0].providedByUserId !== ctx.user!.id && !isAdmin(ctx.user)) {
				throw new TRPCError({ code: 'FORBIDDEN', message: 'Can only delete your own payment records' });
			}

			await db.transaction(async (tx) => {
				await tx.delete(schema.orderPaymentInfo).where(eq(schema.orderPaymentInfo.id, input.id));
