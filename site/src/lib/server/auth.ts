...
export async function validateSessionForChatStream(sessionId: string): Promise<AuthUser | null> {
	const sessionResult = await db
		.select()
		.from(sessions)
		.where(eq(sessions.id, sessionId))
	...
	return {
		...
		username: user.username,
		telegramId: user.telegramId,
		permissions: [...new Set(grants.map((grant) => grant.permissionName).filter((name) => name !== null))],
		roles: [...new Set(grants.map((grant) => grant.roleName))],
		mustChangePassword: user.mustChangePassword
	};
}
...
export async function updateUserPassword(userId: string, newPassword: string): Promise<void> {
	...
	// Invalidate all existing sessions for this user (F1-2: force re-authentication)
