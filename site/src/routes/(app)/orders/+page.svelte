<script lang="ts">
	...
	let displayedOrders = $state<Order[]>([]);
	...
	async function fetchOrders() {
		loading = true;
		try {
			const result = await trpc.orders.listPaginated.query({
				page: currentPage,
				pageSize,
				search: searchQuery || undefined,
				status: statusFilter === 'all' ? undefined : statusFilter,
				markerId: markerFilter === 'all' ? undefined : markerFilter
			});
	...
	onMount(async () => {
		try {
			const [_, markersData] = await Promise.all([
				fetchOrders(),
	...
	<Card.Root>
		<Card.Content class="pt-6">
			<div class="flex flex-col md:flex-row gap-4">
				...
				<Select.Root type="single" bind:value={statusFilter} onValueChange={handleFilterChange}>
				...
				<Select.Root type="single" bind:value={markerFilter} onValueChange={handleFilterChange}>
					<Select.Trigger class="w-full md:w-48">
						...
						<Card.Content class="pt-6">
							<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
								...
								<div class="flex flex-col items-end gap-2">
									<CurrencyPrice amount={order.cost} currency={order.currency || 'USD'} class="text-2xl" />
